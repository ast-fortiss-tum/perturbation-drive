    ...
    def _createDefaultDrop(self):
        ...
        if self.shape == 0:
            ...
            self.alphamap = pyblur.GaussianBlur(
                Image.fromarray(np.uint8(self.labelmap)), self.blur_coeff
            )
        ...
        if self.shape == 2:
            ...
            xys = [
                (self.radius * C[0], self.radius * C[1]),
                (self.radius * B[0], self.radius * B[1]),
                (self.radius * D[0], self.radius * D[1]),
            ]
            ...
            xys = [
                (self.radius * C[0], self.radius * C[1]),
                (self.radius * A[0], self.radius * A[1]),
                (self.radius * D[0], self.radius * D[1]),
            ]
            bezier = make_bezier(xys)
