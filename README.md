...
- `road_gen=True`: A boolean indicating if the road is generated by the benchmarking object. The road generation will be performed based on the current performance of the model under the perturbation on the current road and gradually the roads will become more difficult.
...
The road will be created based on the model performance after each run on the track (TODO: Currently it is random). This will allow the model to start on easy roads (TODO: Define what easy is based on literature) and then gradually work its way up to more difficult roads.
Each road is represented via the Scipted Path Method (TODO: Define it here mathematically) and the road is represented as a list of strings.

If you want to create a standalone Road Generation object you can do so via the `RoadGenerator` class.

```Python
from perturbationdrive import RoadGenerator

road_generator = RoadGenerator()

road_length = 100
road = road_generator.generate_random_road(road_length)
```

What will happen here next

- This class can calculate the road difficulty as number? in an interval?.
- This class has an interface to the perturbation class to measure the current performance (read the logs?) and correlate left and right turns with performance measures (XTE).
- This class can generate a road which is either more difficult or easire than the previous road.
- This class logs all generated roads.
