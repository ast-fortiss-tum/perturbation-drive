...
"""
matplotlib can be a pain to set up. So handle the case where it is absent. When present,
...
def generator(
    ...
    its state and returns just the latest batch required via the yield command.
    ...
    settings. Optionally a shadow image may be overlaid with some random rotation and
    opacity.
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for fullpath in batch_samples:
                try:
                    ...
                    # check if this file exists and only continue if it does
...
def go(model_name, epochs=50, inputs="./log/*.*", limit=None):
    ...
    """
    Load existing model if checkpoint exists, otherwise create a new one.
    """
    if os.path.exists(model_name):
        print("Loading existing model")
        model = keras.models.load_model(model_name)
        # Create a new model name with a timestamp
        base, ext = os.path.splitext(model_name)
        new_model_name = f"{base}_extended5{ext}"
    else:
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(
            new_model_name, monitor="val_loss", save_best_only=True, verbose=0
        ),
    ]
    ...
    print(f"Finished training. Saved model {new_model_name}")


if __name__ == "__main__":
    conf = Conf()
    parser = argparse.ArgumentParser(description="train script")
    parser.add_argument("--model", type=str, default="./checkpoints/original_extended3.h5", help="model name")
    parser.add_argument(
        "--epochs",
        type=int,
        default=1,
