...
class DonkeySimMsgHandler(IMesgHandler):
    ...
    def on_telemetry(self, data):
        ...
        # perturbe the image
        self.img_arr = self.perturbation.gaussian_noise(self.img_arr)
    ...
    def on_parsed_outputs(self, outputs):
        ...
        # random steering for demo reasons
        self.steering_angle = np.random.uniform(-0.2, 0.2) # 0.0
        self.throttle = 0.2
...
def go(filename, address, constant_throttle=0, num_cars=1, image_cb=None, rand_seed=None):
    ...
    for _ in range(0, num_cars):
        # setup the clients
        handler = DonkeySimMsgHandler(constant_throttle, image_cb=image_cb, rand_seed=rand_seed)
