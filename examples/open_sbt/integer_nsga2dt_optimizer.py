...
class IntegerNsgaIIDTOptimizer(Optimizer):
    algorithm_name = ALGORITHM_NAME

    def __init__(self, problem: Problem, config: SearchConfiguration):
    ...
    def run(self) -> SimulationResult:
        ...
        inner_algorithm = NSGA2(
            ...
            sampling=None,
            crossover=SBX(
                prob=prob_crossover, eta=eta_crossover, vtype=int
            ),  # get_crossover("int_sbx", prob=prob_crossover, eta=eta_crossover),
            mutation=PM(
                prob=prob_mutation, eta=eta_mutation, vtype=int
            ),  # get_mutation("int_pm", prob=prob_mutation, eta=eta_mutation),
        ...
        while n_func_evals < config.n_func_evals_lim:
            ...
            for critical_region in critical_regions:
                ...
                initial_population = Population(individuals=nd_individuals_region)
                ...
                self._update_history(
                    res, hist_holder, tree_iteration, inner_num_gen, inner_algorithm
                )

                hist = res.history

                # hist[i] is an object of <class 'pymoo.algorithms.moo.nsga2.NSGA2'>
                best_population_unranked = Population.merge(best_population, res.opt)
                best_population = get_nondominated_population(best_population_unranked)
            ...
            critical_regions = [region for region in regions if region.is_critical]
        ...
        result = self._create_result(
            problem, hist_holder, inner_algorithm, execution_time
        )
        self.res = result
        return result

    def _update_history(
        self, res, hist_holder, tree_iteration, inner_num_gen, inner_algorithm
    ):
        for i in range(inner_num_gen):
            ...
            algo.pop = pop
            opt_pop = Population(individuals=calc_nondominated_individuals(pop))
            algo.opt = opt_pop
